...
namespace DragonBoyZ.Application.Constants
{
    public class Session_ME : ISession_ME
    {
        ...
        public bool LoginGame(string c_username, string c_password, string c_version, sbyte c_type, Message message)
        {
            ...
            try
            {
                ...
                if (player != null)
                {
					...
					string text = $"Username: {username} -- Version: {Version} ";
					Server.Gi().Logger.PrtColor("blue", "Player", "darkblue", text);
                    return true;
                }
                else
                {
        ...
        public void Disconnect()
        {
            ...
            ClearNetwork();
			Server.Gi().Logger.PrtColor("blue","Player","darkblue",$"Session: {Id} Disconnecd.!");
        }
