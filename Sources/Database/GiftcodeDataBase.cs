...
namespace Sources.Database
{
    ...
    public class GiftcodeDataBase
    {
        ...
        public static void RewardGiftcode(Character character, string code)
        {
            ...
            if (character.LengthBagNull() < item.Count)
            {
        ...
        public static bool CanUse(string code)
        {
            lock (Server.SQLLOCK)
            {
                try
                {
                    ...
                    command.CommandText = $"SELECT `code` FROM `giftcode` WHERE `code`= '{code}' AND count > 0 AND time_expire >= CURRENT_TIMESTAMP";
                    using var reader = command.ExecuteReader();
                    if (reader.HasRows)
                    {
                        return true;
        ...
        public static int GetThoiVang(string code)
        {
            lock (Server.SQLLOCK)
            {
                try
                {
                    ...
                    command.CommandText = $"SELECT `gold` FROM `giftcode` WHERE `code` = '{code}' AND count > 0 AND time_expire >= CURRENT_TIMESTAMP";
                    using var reader = command.ExecuteReader();
        ...
        public static int GetGem(string code)
        {
            lock (Server.SQLLOCK)
            {
                try
                {
                    ...
                    using DbCommand command = DbContext.gI()?.Connection.CreateCommand();
                    if (command == null) return -1;
                    command.CommandText = $"SELECT `gem` FROM `giftcode` WHERE `code` = '{code}' AND count > 0 AND time_expire >= CURRENT_TIMESTAMP";
                    using var reader = command.ExecuteReader();
                    if (reader.HasRows && reader.Read())
                    {
                        return reader.GetInt32(0);
        ...
        public static int GetRuby(string code)
        {
            lock (Server.SQLLOCK)
            {
                try
                {
                    ...
                    using DbCommand command = DbContext.gI()?.Connection.CreateCommand();
                    if (command == null) return -1;
                    command.CommandText = $"SELECT `ruby` FROM `giftcode` WHERE `code` = '{code}' AND count > 0 AND time_expire >= CURRENT_TIMESTAMP";
                    using var reader = command.ExecuteReader();
                    if (reader.HasRows && reader.Read())
                    {
                        return reader.GetInt32(0);
