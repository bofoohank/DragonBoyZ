...
namespace DragonBoyZ.Application.Handlers.Monster
{
        ...
        public void LeaveItem(ICharacter character)
        {
            ...
            }else if (Monster.Id == 70)
            {
                for (int i = 0; i < 20; i++)
                {
                    ...
                    var itemMap = new ItemMap(character.Id, item);
                    itemMap.X = (short)(Monster.X + (i * 10));
                    itemMap.Y = Monster.Y;
                    Monster.Zone.ZoneHandler.LeaveItemMap(itemMap);
                }
                if (character.InfoChar.ThoiGianTrungMaBu <= 0)
                {
                    ...
                    character.CharacterHandler.SendMessage(Service.ServerMessage("Bạn đã giết được Hirudergan, hãy về nhà và kiểm tra phần thưởng của bạn"));
                }
                if (ServerUtils.RandomNumber(100) < 5)
                {
                    var itemMap = LeaveItemHandler.LeaveGodItem(character);
                    itemMap.X = Monster.X;
                    itemMap.Y = Monster.Y;
                    Monster.Zone.ZoneHandler.LeaveItemMap(itemMap);
                }
