...
using static DragonBoyZ.Application.Extension.Super_Champion.SieuHang;
using System.IO;
...
namespace DragonBoyZ.DatabaseManager.Player
{
    public class CharacterDB
    {
            ...
            lock (Server.SQLLOCK)
            {
                try
                {
                   ...
                   // text += $", `DataKyGUI` = '{JsonConvert.SerializeObject(character.DataKyGUI)}'";
                    ...
                    command.CommandText = $"UPDATE `character` SET {text}  WHERE `id` = {character.Id};";
                    command.ExecuteNonQuery();
					string text1 = "Save character " + character.Name + " -- id: " + character.Id + " -- success !";
					Server.Gi().Logger.PrtColor("yellow", "Update", "darkyellow", text1);
                }
                catch (Exception e)
                {
                    string text2 = "Save character " + character.Name + " -- id: " + character.Id + " -- error !";
					Server.Gi().Logger.PrtColor("yellow", "Update", "darkred", text2);
        ...
        public static void SelectBXHSucManh(int limit)
        {
            lock (Server.SQLLOCK)
            {
                try
