...
namespace DragonBoyZ.DatabaseManager
{
    public class InitData
    {
            ...
            using (DbCommand command = DbContext.gI()?.Connection.CreateCommand())
            {
                if (command != null)
                {
                    ...
                    if (reader.HasRows)
                    {
						...
						Server.Gi().Logger.PrtColor("red", "Server","white","Load Cai Trang: " + Cache.Gi().CaiTrangTemplate.Count);
                    }
                    reader.Close();
                    #endregion
					...
					Server.Gi().Logger.PrtColor("red", "Server","white","Load Monster: " + Cache.Gi().MONSTER_TEMPLATES.Count);
                    reader.Close();
					...
					Server.Gi().Logger.PrtColor("red", "Server","white","Load Npc: " + Cache.Gi().NPC_TEMPLATES.Count);
                    reader.Close();
					...
					Server.Gi().Logger.PrtColor("red", "Server","white","Load Boss: " + Cache.Gi().BOSS_TEMPLATES.Count);
                    reader.Close();
                    ...
                    Server.Gi().Logger.PrtColor("red", "Server","white","Load Item: " + Cache.Gi().ITEM_TEMPLATES.Count);					
                    reader.Close();
                    ...
                    Server.Gi().Logger.PrtColor("red", "Server","white","Load Map: " + Cache.Gi().TILE_MAPS.Count);					
                    reader.Close();
                    #endregion
        ...
        {
            Cache.ClearCache();
			//Server.Gi().Logger.SvPrint("Clear Cache","red");         
        }
        ...
        private static IEnumerable<sbyte> SetupVersionIcon()
        {
            ...
            data = writer.GetData().ToList();
            writer.Close();
			Server.Gi().Logger.PrtColor("red", "Server","white","Load Icon: " + iconMax);
            return data;
        }
    }
}