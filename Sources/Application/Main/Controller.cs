...
using DragonBoyZ.Application.Handlers.Client;
using DragonBoyZ.Application.Handlers.Item;
using DragonBoyZ.Application.Handlers.Skill;
using DragonBoyZ.Application.Interfaces.Character;
using DragonBoyZ.Application.Interfaces.Client;
...
using DragonBoyZ.Model.Data;
using DragonBoyZ.Application.Handlers.Menu;
using DragonBoyZ.Application.Extension.Super_Champion;
using DragonBoyZ.Application.Extension.Bo_Mong;
                        ...
                        if (character == null) return;
                            if (character.IsNextMap)
                            {
                                ...
                                if (zone != null)
                                {
                                    ...
                                    //character.CharacterHandler.SendZoneMessage(Service.SendImageBag(character.Id, DragonBoyZ.Sources.Application.Extension.Namecball.NamecBallHandler.SendBagNamecBall(character)));
                                }
                    ...
                    case 44:
                        {
                            ...
                            if (text.Contains("cauca") && QuanLyServer.Start_EventCauCa)
                            {
                                ...
                                {
                                    var itemCancau = character.CharacterHandler.GetItemBagById(1290);
                                    if (itemCancau != null)
                                    {
                                        if (character.InfoChar.MapId == 166)//Check map
                                        {
                                            if (character.InfoChar.X >= 227 && character.InfoChar.Y >= 456)
                                            {
                                                character.CharacterHandler.SendMessage(Service.OpenUiConfirm(80, MenuNpc.Gi().TextCauCa[1], MenuNpc.Gi().MenuCauCa[1], character.InfoChar.Gender));
                                                character.TypeMenu = 1;
                                            }
                                            else
                                            {
                                                character.CharacterHandler.SendMessage(Service.OpenUiSay(5, "Bạn phải xuống nước mới có thể sử dụng cần câu cá", false, character.InfoChar.Gender));
                                            }
                                        }
                                        else
                                        {
                                            character.CharacterHandler.SendMessage(Service.OpenUiSay(5, "Bạn phải ở Bờ biển Kame mới có thể sử dụng cần câu cá", false, character.InfoChar.Gender));
                                        }
                                    }
                                    else
                                    {
                                        character.CharacterHandler.SendMessage(Service.OpenUiSay(5, "Bạn không có cần câu trong người", false, character.InfoChar.Gender));
                                    }
                                }
                                else
                                {
                                    character.CharacterHandler.SendMessage(Service.OpenUiSay(5, "Vui lòng chờ vài giây", false, character.InfoChar.Gender));
                                }
                            }
                            #endregion
                            ...
                            if (!text.Equals("") && character != null && role == 1)
                            {
                                ...
                                {
                                    string textAdmin = $"{ServerUtils.Color("red")}MENU ADMIN" 
                                        + $"{ServerUtils.Color("blue")}Xin chào {_session.Player.Character.Name}" 
                                        + $"{ServerUtils.Color("green")}Online: {ClientManager.Gi().Players.Count} / Sesions: {ClientManager.Gi().Sessions.Count} / Thread: {System.Diagnostics.Process.GetCurrentProcess().Threads.Count}";
                                
                                    character.CharacterHandler.SendMessage(Service.OpenUiConfirm(64, textAdmin, MenuNpc.Gi().MenuAdmin[0], character.InfoChar.Gender));
                                ...
                                #region Lock clone giao dich SPL
                            ...
                            case 50:
                                {
                                    ...
                                    arrIndexUi.ForEach(index =>
                                    {
                                        ...
                                        if (ItemCache.ItemTemplate(itemIndex.Id).Level == 14) indexItemHuyDiet = itemIndex.IndexUI;
                                    });
                                    if (indexItemHuyDiet == -1)
                                    {
                                        character.CharacterHandler.SendMessage(
                                       Service.DialogMessage(TextServer.gI().TRANG_BI_ERROR_2));
                                        return Task.CompletedTask;
                                    }
                                    var item = character.CharacterHandler.GetItemBagByIndex(indexItemHuyDiet);
                                    var itemTemplate = ItemCache.ItemTemplate(item.Id);
                                    var textUpgrade = "|0|" + itemTemplate.Name + "\n";
                                    textUpgrade += "|6|Ti le 50% thanh cong khi that bai\nban co the nhan duoc ngoc rong ngau nhien tu 3 -> 5 sao\n";
                                    textUpgrade += "|6|Can 500 trieu vang";
                                    var enoughGold = false;
                                    if (character.InfoChar.Gold >= 500000000) enoughGold = true;
                                    character.CharacterHandler.SendMessage(Service.OpenUiConfirm(21, textUpgrade, new List<string> { enoughGold ? "Nâng cấp" : "Tu choi" }, character.InfoChar.Gender));
                                    character.TypeMenu = enoughGold  ? 50 :- 1;
                                    character.CombinneIndex = new List<int> { indexItemHuyDiet };
                                    break;
                                }
                            case 21:
                                {
